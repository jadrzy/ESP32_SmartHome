...
static char *cutter(char *string)
{
    char *ptr_cut;
    static char output[64];
    ptr_cut = strchr(string, '=');
    ptr_cut++;
    if (ptr_cut != NULL)
        strncpy(output, ptr_cut, 64);
    else
        strncpy(output, "", 1);
    ...
    while(output[i] != '\0')
    {
        if (!isalnum( (unsigned int) output[i] ) )
        {
            output[i] = '\0';
            break;
        }
...
esp_err_t recieve_html(httpd_req_t *req) {
    ...
    ESP_LOGI(TAG_WIFI, "Http server return");
    ...
    while (1)
    {
        ...
        if (err != ESP_OK) {
            ...
            httpd_resp_send_404(req); // return 404 in case of error
            return ESP_FAIL;
        }
    }

    // Parser
    static char (*data_parsed)[96]; 
    if (data_parsed == NULL)
        data_parsed = malloc(((NUMBER_OF_DEVICES * 2) + 2) * sizeof(char) * 96);

    char *token;
    if ((token = strtok(html_content, "&")) != NULL)
    {
        int i = 0;
        strncpy(*(data_parsed + i), token, 96);

        while ((token = strtok(NULL, "&")) != NULL)
        {
            i++;
            strncpy(*(data_parsed + i), token, 96);
            if (i == ((NUMBER_OF_DEVICES * 2) + 2 - 1))
            {
    ...
    char psswd[64] = "";
    char serial_numbers[NUMBER_OF_DEVICES][SERIAL_NUMBER_SIZE];
    uint64_t mac_addresses[NUMBER_OF_DEVICES] = {0};
    char placeholder[64];

    // ssid
    strncpy(ssid, cutter( *(data_parsed + 0)), sizeof(ssid));


    // psswd
    strncpy(psswd, cutter( *(data_parsed + 1)), sizeof(psswd));

    
    // serials and macs
    for (int i = 0; i < NUMBER_OF_DEVICES; i++)
    {
        strncpy(serial_numbers[i], cutter( *(data_parsed + 2 + (i * 2))), sizeof(serial_numbers[i]));
        strncpy(placeholder, cutter( *(data_parsed + 2 + (i * 2) + 1)), 64);

        int j = 0;
        bool correct_mac = true;

        while(*(placeholder + j) != '\0')
        {
            if (!isdigit( (unsigned int) *(placeholder + j) ) )
            {
                correct_mac = false;
                break;
            }
        ...
        if (correct_mac == true && *(placeholder) != '\0')
        {
            mac_addresses[i] = strtoull(placeholder, NULL, 0);
        }
    }
    ...
    for (int i = 0; i < NUMBER_OF_DEVICES; i++)
    {
        bool serial_validated = false;
        bool mac_validated = false;

        if (strlen(serial_numbers[i]) == 12)
        {
            serial_validated = true;
        }

        char string[24];
        sprintf(string, "%"PRIx64, mac_addresses[i]);
        if (strlen(string) == 12)
            mac_validated = true;

        if (!serial_validated || !mac_validated)
        {
            strcpy(serial_numbers[i], "");
            mac_addresses[i] = 0;
        }
    }
